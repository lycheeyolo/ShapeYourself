# ShapeYourself
记录利用Curosr开发一款“塑型”app.

1. 初始提示词
```plan text
我想创建一款app，名字叫“塑型”。主要的功能包括：
1. 用户创建增重/减重/运动计划。用户输入当前的身高体重等信息，以及期望的目标，大模型根据这个个人信息，生成一份运动计划。
2. 用户每日根据生成的计划打卡，记录计划中每一项的完成情况。这些数据用户可以选择保存到手机本地，也可以选择保存到云端数据库（代码中需要由环境变量配置数据库）。
3. 上面提到的大模型调用，也需要在代码中配置调用相关的变量。

我希望有2-3个界面，最右边是用户界面，包括一些基础信息，以及上面涉及到的所有数据是否保存到云端数据库，还是本地手机。同时允许用户配置自己的大模型调用接口。
主界面是打开信息记录界面，可以包括一些数据的变化图标。
可以根据你的理解增加其他有用的功能
```
